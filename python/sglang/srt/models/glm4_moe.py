...
class Glm4MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Debug: Check for NaN at forward entry (only for layer 4)
        if self.layer_id == 4 and torch.any(torch.isnan(hidden_states)):
            logger.error(
                f"NaN detected at forward entry in layer 4! "
        ...
        # Debug: Check for NaN before attention (only for layer 4)
        if self.layer_id == 4 and torch.any(torch.isnan(hidden_states)):
            logger.error(
                f"NaN detected before self_attn in layer 4 (after prepare_attn)! "
                f"shape={hidden_states.shape}, "
                f"dtype={hidden_states.dtype}, "
                f"num_nan={torch.sum(torch.isnan(hidden_states)).item()}"
            )
        ...
        # Debug: Check for NaN after attention (only for layer 4)
        if self.layer_id == 4 and torch.any(torch.isnan(hidden_states)):
            logger.error(
                f"NaN detected after self_attn in layer 4! "
                f"shape={hidden_states.shape}, "
                f"dtype={hidden_states.dtype}, "
                f"num_nan={torch.sum(torch.isnan(hidden_states)).item()}"
            )
        ...
        # Debug: Check for NaN after mlp (only for layer 4)
        if self.layer_id == 4 and torch.any(torch.isnan(hidden_states)):
            logger.error(
                f"NaN detected after mlp in layer 4! "
                f"shape={hidden_states.shape}, "
                f"dtype={hidden_states.dtype}, "
                f"num_nan={torch.sum(torch.isnan(hidden_states)).item()}, "
