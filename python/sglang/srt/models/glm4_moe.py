...
class Glm4MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Debug: Check for NaN at forward entry (only for layer 1)
        if self.layer_id == 1 and torch.any(torch.isnan(hidden_states)):
            logger.error(
                f"NaN detected at forward entry in layer 1! "
        ...
        # Debug: Check for NaN before attention (only for layer 1)
        if self.layer_id == 1 and torch.any(torch.isnan(hidden_states)):
            logger.error(
                f"NaN detected before self_attn in layer 1 (after prepare_attn)! "
                f"shape={hidden_states.shape}, "
                f"dtype={hidden_states.dtype}, "
                f"num_nan={torch.sum(torch.isnan(hidden_states)).item()}"
            )
        ...
        # Debug: Check for NaN after attention (only for layer 1)
        if self.layer_id == 1 and torch.any(torch.isnan(hidden_states)):
            logger.error(
                f"NaN detected after self_attn in layer 1! "
                f"shape={hidden_states.shape}, "
                f"dtype={hidden_states.dtype}, "
                f"num_nan={torch.sum(torch.isnan(hidden_states)).item()}"
            )
        ...
        # Debug: Check for NaN after mlp (only for layer 1)
        if self.layer_id == 1 and torch.any(torch.isnan(hidden_states)):
            logger.error(
                f"NaN detected after mlp in layer 1! "
                f"shape={hidden_states.shape}, "
                f"dtype={hidden_states.dtype}, "
                f"num_nan={torch.sum(torch.isnan(hidden_states)).item()}, "
